...
public class CacheKeyGenerator {
    ...
    public static String generateCacheKey(@Nullable String keyword, @Nullable Long storeId, int size) {
        StringBuilder keyBuilder = new StringBuilder();

        keyBuilder.append("keyword-")
                .append(keyword != null ? keyword : "all") // <-- cursor.keyword() 대신 keyword 직접 사용
                .append("-");

        if (storeId != null) { // <-- cursor.storeId() 대신 storeId 직접 사용
            keyBuilder.append("storeId-").append(storeId).append("-");
        }
